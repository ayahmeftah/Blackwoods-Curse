...
{
    public float angleRotationY = 181.559f;
    ...
    void Start()
    {
        ...
        textCanvasGroup.alpha = 0f; // Hide UI initially
    }

    void Update()
    {
        if (isPlayerNearby && mirrorController.isSecondMirrorRotated && !isRotated)
        {
            canRotate = true;
    ...
    {
        if (other.CompareTag("Player") && mirrorController.isSecondMirrorRotated && !isRotated)
        {
            isPlayerNearby = true;
